...
from common.error import NoUserError, UserActiveError, RoleError
...
class Admin(Base):
    ...
    def update_gift(self, first_level, second_level,
        ...
        self.__check('No permission.')
        self._Base__update_gift(first_level, second_level,
                                gift_name, gift_count, is_admin=True)


if __name__ == '__main__':
    gift_path = os.path.join(os.getcwd(), 'storage', 'gift.json')
    user_path = os.path.join(os.getcwd(), 'storage', 'user.json')
    admin = Admin('peter', user_path, gift_path)
    admin.update_gift(first_level='level1', second_level='level2',
                      gift_name='iphone 12', gift_count=30)
